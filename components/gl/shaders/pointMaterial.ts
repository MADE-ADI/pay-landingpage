  ...
  constructor() {
    super({
      ...
      void main() {
        ...
        // Create orange gradient based on position and time
        vec3 orangeColor1 = vec3(1.0, 0.42, 0.21);  // #FF6B35
        vec3 orangeColor2 = vec3(0.97, 0.58, 0.12); // #F7931E  
        vec3 orangeColor3 = vec3(1.0, 0.71, 0.30);  // #FFB84D
        vec3 whiteColor = vec3(1.0, 1.0, 1.0);
        
        // Use world position for gradient variation
        float gradientFactor = sin(vWorldPosition.x * 0.1 + uTime * 0.3) * 0.5 + 0.5;
        float heightFactor = (vWorldPosition.y + 5.0) / 10.0; // Normalize height
        
        // Mix orange colors based on position and time
        vec3 orangeMix = mix(orangeColor1, orangeColor2, gradientFactor);
        orangeMix = mix(orangeMix, orangeColor3, heightFactor);
        
        // Create final color: mostly white with orange tint
        float orangeInfluence = 0.3 + sin(uTime * 0.2) * 0.1; // Subtle animation
        vec3 finalColor = mix(whiteColor, orangeMix, orangeInfluence);
