...
const VignetteTimeUpdater = ({ darkness, offset }: { darkness: number; offset: number }) => {
  const vignetteRef = useRef<any>(null);
  
  useFrame((state) => {
    if (vignetteRef.current) {
      vignetteRef.current.uniforms.uTime.value = state.clock.elapsedTime;
    }
  });
    ...
    <div id="webgl">
      ...
      >
        ...
        <Effects multisamping={0} disableGamma>
          <VignetteTimeUpdater darkness={vignetteDarkness} offset={vignetteOffset} />
        </Effects>
      </Canvas>
    </div>
  );
