...
export const Pill = ({ children, className }: { children: React.ReactNode, className?: string }) => {
  ...
  return (
    <div
      ...
      className={cn("bg-[#262626]/50 transform-gpu font-medium text-foreground/50 backdrop-blur-xs font-mono text-xs sm:text-sm inline-flex items-center justify-center px-2.5 sm:px-3 h-7 sm:h-8 border border-border [clip-path:polygon(var(--poly-roundness)_0,calc(100%_-_var(--poly-roundness))_0,100%_var(--poly-roundness),100%_calc(100%_-_var(--poly-roundness)),calc(100%_-_var(--poly-roundness))_100%,var(--poly-roundness)_100%,0_calc(100%_-_var(--poly-roundness)),0_var(--poly-roundness))]", className)}
    >
      ...
      <span className="inline-block size-2 sm:size-2.5 rounded-full bg-primary mr-1.5 sm:mr-2 shadow-glow shadow-primary/50" />
